...
def getINTMMatrix(bsize, rng, igeon_pose):
	...
	# incorrect: igeo_pose = T.batched_dot(fix_center_to_origin, T.batched_dot(trans_matrix, T.batched_dot(scaling_matrix, T.batched_dot(shearing_matrix, T.batched_dot(rot_matrix, fix_corner_to_origin)))))

	igeo_pose, _ = theano.map(lambda i: build_single_iGeoPose(igeon_pose[i, :]),
